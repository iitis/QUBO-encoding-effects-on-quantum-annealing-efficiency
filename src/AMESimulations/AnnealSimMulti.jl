...
fn_output = isempty(parsed_args["name"]) ? "MultiData.bin" : parsed_args["name"] * "_MultiData.bin"
...
# Set up the problem
###
print("Reading list of problems...")
problem_list = LoadManyQUBOs(parsed_args["srcfile"])
@printf("Done! File contains %d QUBOs.\n", length(problem_list))
...
#results_list = Vector{Any}()
#for (the_psum, the_ppair, problem) in ProgressBar(problem_list)
...
results_list = Vector{Any}(undef, length(problem_list))
Threads.@threads :greedy for idx in ProgressBar(1:length(problem_list))
    problem = problem_list[idx].qubo

    annealing = SetUpAnnealingProblem(problem, schedule, parsed_args)
    solution_pops = parsed_args["solve-ame"] ? SolveWithAME(annealing, tf, s_range) : SolveWithSE(annealing, tf, s_range)

    results_list[idx] = (
        psum = problem_list[idx].psum,
        ppair = problem_list[idx].ppair,
        final_results = ExtractFinalResults(problem, solution_pops)
    )
end
