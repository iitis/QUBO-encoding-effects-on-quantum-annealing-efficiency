...
for psum in np.logspace(-1, 1, SIM_NPTS):
    for ppair in np.logspace(-1, 1, SIM_NPTS):
        ...
        if SIM_AME and SIM_REVERSE:
            command_list.append(f"julia ./AnnealSaveIntermediateStates.jl {fn} -n {nm} -t {SIM_TANNEAL} --beta {SIM_BETA} --reverse-anneal-point 0.5 --solve-ame")
        elif SIM_AME:
